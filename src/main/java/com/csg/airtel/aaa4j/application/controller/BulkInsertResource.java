 ...
 *
 * Note: MAC_ADDRESS is stored in a separate table AAA_USER_MAC_ADDRESS with fields:
 * ID, USER_NAME, MAC_ADDRESS (normalized), ORIGINAL_MAC_ADDRESS, CREATED_DATE, UPDATED_DATE
 *
 * Constraints:
 * - USER_ID: Primary Key
...
public class BulkInsertResource {
     ...
     * CONTACT_EMAIL, CONTACT_NAME, CONTACT_NUMBER, CREATED_DATE, CUSTOM_TIMEOUT,
     * CYCLE_DATE, ENCRYPTION_METHOD, GROUP_ID, IDLE_TIMEOUT, IP_ALLOCATION, IP_POOL_NAME,
     * IPV4, IPV6, NAS_PORT_TYPE, PASSWORD, REMOTE_ID, REQUEST_ID (UNIQUE),
     * SESSION_TIMEOUT, STATUS, UPDATED_DATE, USER_NAME (UNIQUE), VLAN_ID, NAS_IP_ADDRESS, SUBSCRIPTION
     *
     * Note: MAC_ADDRESS data is stored separately in AAA_USER_MAC_ADDRESS table
     */
    @POST
    @Path("/setup")
    ...
    @Path("/ids")
    @Produces(MediaType.APPLICATION_JSON)
    public Uni<Response> getAllIds() {
        // Fire the async operation
        Uni.createFrom().voidItem()
                .emitOn(executor)
                .subscribe().with(
                        unused -> bucketInstanceDataGenerator.generateDataBucket()
                                .subscribe().with(
                                        result -> LOG.info("Generation completed: " + result),
                                        failure -> LOG.error("Generation failed", failure)
                                )
                );

        // Return immediate response
        return Uni.createFrom().item(
                Response.accepted().entity(Map.of("message", "Generation started")).build()
        );
    }
}
